...
def compact_payload(payload: dict[str, Any]) -> str:
    try:
        ...
        if "audio" in shallow and isinstance(shallow["audio"], str):
            if len(shallow["audio"]) > 64:
                shallow["audio"] = shallow["audio"][:64] + "..."
    ...
    async def _connect(self) -> None:
        async with self._connect_lock:
            ...
            self.logger.log_info(
                f"[QWEN-ASR] connected to realtime endpoint in "
    ...
    async def _send_json(self, payload: dict[str, Any]) -> None:
        ...
        async with self._send_lock:
            ...
            self.logger.log_debug(
                f"[QWEN-ASR] -> {compact_payload(payload_to_send)}"
            )
        ...
        try:
            async for raw_message in websocket:
                if isinstance(raw_message, bytes):
                    self.logger.log_debug(
                        f"[QWEN-ASR] ignore binary frame len={len(raw_message)}"
                    )
                    continue
                ...
                self.logger.log_debug(
                    f"[QWEN-ASR] <- {compact_payload(payload)}"
                )
