...
func (w *Worker) start(req *StartReq) (err error) {
	...
	pid := cmd.Process.Pid
	slog.Info("Worker startup probe", "requestId", req.RequestId, "pid", pid, logTag)

	// In our packaged runtime `tman run start` may stay as the foreground process instead
	// of forking a stable child under the same parent PID. Treat a still-alive process group
	// as a successful start instead of blocking the HTTP `/start` path for 10 seconds.
	started := false
	for i := 0; i < 3; i++ {
	...
	if !started {
		waitErr := cmd.Wait()
		if waitErr != nil {
			slog.Error("Worker exited during startup", "err", waitErr, "requestId", req.RequestId, logTag)
			return waitErr
		}
	...
	w.Pid = pid
	slog.Info("Worker startup confirmed", "requestId", req.RequestId, "pid", pid, logTag)
