...
func (s *HttpServer) processProperty(req *StartReq, tenappDir string) (propertyJsonFile string, logFile string, err error) {
	...
	if s.config.AppCertificate != "" {
		tokenUid := req.BotStreamId
		// RTM validates the user identity encoded in the token. The worker logs in as
		// the bot UID, so the worker-side token must be minted for bot_uid instead of 0.
		req.Token, err = rtctokenbuilder.BuildTokenWithRtm(
			s.config.AppId,
			s.config.AppCertificate,
			req.ChannelName,
			fmt.Sprintf("%d", tokenUid),
...
func (s *HttpServer) Start() {
	...
	if err := r.SetTrustedProxies(nil); err != nil {
		slog.Warn("failed to configure trusted proxies", "err", err, logTag)
	}
