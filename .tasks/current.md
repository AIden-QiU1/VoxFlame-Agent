...
- 用户提供 79 秒 Android 真机录屏。逐帧证据显示：进入“看病与求助”后，录音卡位于长页面下方；点击“开始说这句话”只弹出“先完成采集前确认”，但 3 个确认项位于数屏之外的页面顶部，当前视口没有可执行动作。同时“下一句”仍可从 1/56 跳到 2/56，因此用户可以切题却不能开始录音。
- 根因是前置状态和主动作不在同一交互上下文：`collectionPreflightReady` 只在开始函数中弹窗拦截，上一句/下一句未共享同一门禁；录音页还重复渲染完整材料选择、任务说明和可选字段。App 根部复用单一 ScrollView，任务路由改变时没有重置滚动位置，会进一步把用户带到页面中段。
- 已将 3 项确认移入目标句卡和主录音按钮之间；未完成时按钮明确显示“先完成上方确认”，前后切题同步禁用，完成后本组持续有效。路由/一级页面切换会自动滚到顶部；录音页不再重复展示全部题库入口，可选年龄/性别移到录音卡之后；Android 根安全区补入状态栏高度。
- 新增 `getMobileCollectionControlState` 和回归脚本，锁定未确认时主动作/导航门禁与确认后的解锁行为；静态检查同时禁止恢复仅弹窗的隐藏前置路径，并要求共享 ScrollView 在任务路由变化时回顶。
- 验证通过：`npm run typecheck`、`npm run test:training`、`npm run test:communication`、`npm run test:memory`、`npm run check`、Android Expo export、iOS Expo export、`git diff --check`。宿主仍继承已知的 `NODE_TLS_REJECT_UNAUTHORIZED=0` 警告；本次未创建 EAS 云构建、未发布 APK/iOS 包，Android 真机仍需复验首次进入回顶、3 项确认、开始/停止录音、确认收录和连续下一句。

## 2026-08-31 简洁版采集说明

- 根据用户要求，将普通话语音采集说明改为尽可能简洁的页面使用语言。
- 删除接口、代码、内部编号、音频参数、保存凭据、验收等级和其他工程术语。
- 仅保留开始前准备、录音步骤、页面提示、注意事项、保留/重录判断、文章录音和陪同话术。
- 已重新生成根目录 DOCX，排版为 A4 两页；`bash scripts/check_ai_docs.sh` 通过。

## 2026-08-31 采集主流程标识减法与 SOP v1.1

- 用户将本轮重点明确为“清理多余的标识”。普通引导式数据录入页现只保留当前任务、必要保存状态、题面、系统听到、回听和下一步操作；已移除三步进度导航、本轮完成徽章、顶部录音时长卡、账号句子统计卡、重复步数、“目标句”“录音成功”、自动任务说明卡及重复质量摘要。评估页和非引导流程中的业务字段未做横向误删。
- 原 `燃言_构音障碍普通话语音采集标准化SOP_v1.0.docx` 保留不变；新增 [Web 对齐版 Markdown](../docs/燃言_构音障碍普通话语音采集标准化SOP_v1.1_Web对齐版.md)、可重复生成脚本 `scripts/build_voice_collection_sop_v1_1.py` 和根目录 v1.1 DOCX。生成结果为有效 OOXML、A4、5 页，未残留 Markdown 标记，内容与网站逐句录音、自动保存、本机补登、严格替换重录和“构音差异不强制重录”边界对齐。
- 新增源码回归测试锁定冗余标识不会重新进入普通引导式采集流。验证通过：定向 5 项、Frontend 全量 121 项、`npx tsc --noEmit`、Next production build（25/25）、`bash scripts/check_ai_docs.sh` 和 `git diff --check`。
- Playwright 未登录 smoke：访问 `/contribute/topic/daily-life` 正确回跳 `/login?next=%2Fcontribute%2Ftopic%2Fdaily-life`，console 0 error/0 warning。当前未部署；本机没有真实登录态和麦克风设备，因此已登录后的标识视觉、连续录制与保存状态仍需真实账号设备 smoke。

## 2026-08-31 下一句录音被上一句上传锁住

- 用户复现为：第 1 句停止后页面已自动切到第 2 句、状态回到等待开始，但录音按钮仍禁用；刷新后恢复。代码证据确认这不是题目游标、麦克风权限或 RTC 连接问题，而是训练页把 hook 级全局 `isUploading` 同时用于后台持久化和录音按钮互斥。
- 状态边界已修正：`isProcessing` 只覆盖当前录音停止后的本地音频/最终转写整理；上一句 `persistTrainingAttempt()` 可以在后台继续，不能阻止下一句采集或示例朗读。两个训练布局都复用同一录音控制规则，并有“后台持久化不是禁用依赖”的回归测试。
- 上传链路的签名、OSS PUT、完成登记统一增加 20 秒单请求上限；超时/网络异常继续复用现有本机 recorder queue，网络恢复后补登。并发上传使用活动计数维护 `isUploading`，避免较早请求完成时把仍在运行的请求错误标成空闲。
- 验证：新增录音控制与上传超时 4 项定向测试；连同本轮标识回归后 Frontend 全量 121 项、`npx tsc --noEmit`、Next production build（25/25）和 `git diff --check` 通过。Playwright 访问受保护采集路由正确回跳并保留 `next`，console 0 error/0 warning。
- 当前未部署；本机没有可复用真实登录态和麦克风设备，因此仍需账号 308 在发布后连续录 2–3 句，确认第 2 句无需刷新即可开始，并观察慢网/断网时前一条进入本机待同步队列。浏览器工具仍继承宿主 `NODE_TLS_REJECT_UNAUTHORIZED=0`，发布前的既有移除门保持不变。
