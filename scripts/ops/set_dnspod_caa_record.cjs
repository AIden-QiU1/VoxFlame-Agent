...
const SUB_DOMAIN = process.env.DNSPOD_CAA_SUB_DOMAIN || '@'
const RECORD_TYPE = 'CAA'
const RECORD_LINE = process.env.DNSPOD_RECORD_LINE || '默认'
const RECORD_VALUE = process.env.DNSPOD_CAA_VALUE || '0 issue "letsencrypt.org"'
...
async function main() {
  console.log(`Checking DNSPod CAA record: ${SUB_DOMAIN}.${DOMAIN} -> ${RECORD_VALUE}`)
  ...
  const targetRecord = existingRecords.find(
    (record) => record.Type === RECORD_TYPE && record.Value === RECORD_VALUE,
  )

  if (DRY_RUN) {
    console.log(JSON.stringify({ existingRecords, targetRecord: targetRecord || null }, null, 2))
    return
  }

  if (targetRecord) {
    console.log(`CAA record already present, id=${targetRecord.RecordId}`)
    return
  }
