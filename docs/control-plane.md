...
- Participant token metadata/attributes 不携带认证用户 ID 或 ASR 账户键；Agent 不再从 participant metadata、attributes 或缺失路由时的身份兜底猜测模型账户，只信任 Backend 创建的 dispatch metadata。
...
- 审计期望保存在 `backend/config/asr-model-audit-expectations.json`，只用于部署/升级核验，不参与运行时路由；运行时唯一模型事实源仍是 8001 注册表。
- 最小 live smoke：运行 `cd backend && env -u NODE_TLS_REJECT_UNAUTHORIZED npm run audit:asr-models`，逐一确认真实 Supabase 注册用户、Backend 推导的 `asr_account_id`、8001 注册表和真实 WAV 推理模型完全一致；三个个性化账户必须返回 `personalized=true/fallback=false`，随机未注册键必须返回 `personalized=false/fallback=true`。另需验证 8001 故障时 DashScope 回退。
